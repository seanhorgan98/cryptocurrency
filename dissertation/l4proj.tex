...
[\ref{fig:blockchainDefinition}]. Every time a transaction is processed by a node the outputs that are created 
...
\begin{figure}[!ht]
    \centering
    \hfill\includegraphics[width=1\linewidth]{images/blockchainDefinition.png}    

    \caption{
        This figure shows the definitions for the blockchain and also the UTXO maps. The blockchain is an ArrayList
        created with one starting block acting as a genesis block. The UTXO map is a HashMap mapping the transaction
        id to the transaction output created from each transaction.
    }

    % use the notation fig:name to cross reference a figure
    \label{fig:blockchainDefinition} 
\end{figure}
...
The tests that each node would run are in relation to dealing with new transactions when they are flooded through
...
%Genesis blocks
%Test Harness node generation
%Wallets being created from nodes
