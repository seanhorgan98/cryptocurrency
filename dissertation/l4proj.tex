...
choosing one particular design instead of another any why those choices were made.

\section{Overview}
The main aspects of the design process that needed to be addressed were surrounding transactions, the blockchain, and
...
recieved and also contain the necessary information like the value of currency that transaction is sending.

%signatures
One of the key functionality of a transaction would be it's ability to generate a signature for itself using the values
from the various metadata it contained and then it also has to be able to vefify that this signature is valid. This
is a basic building block for what makes a cryptocurrency secure.
...
way we decided to enforce this condition was through the use of Transaction inputs and outputs. When a transaction is
sent, the value of that transaction (amount of currency it is trying to send), is a collection of inputs. These inputs
...
are addressed to that particular user. This concept will be explained in more detail in the Blockchain section.

\subsection{Process and Checks}
%Process transaction
When a transaction is sent it has to be processed before it is considered valid and added to a block and subsequently
the blockchain.

%Coin creation
When is created it can be one of two types. The first type is that of a normal transaction which would take place
